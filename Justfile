...
release-candidate:
    ...
    # Downstream crates depend on unpublished sibling 0.9 packages until the publish
...
runtime-smoke:
    ...
    cargo run -p mdx-rust -- schema agent-runtime-manifest --json
    cargo run -p mdx-rust -- schema agent-pack --json
    printf '%s\n%s\n' '{"id":1,"method":"tools/list"}' '{"id":2,"method":"tools/call","params":{"name":"evolve","arguments":{"apply":true}}}' | cargo run -p mdx-rust -- mcp --stdio | python3 -c 'import json,sys; lines=[json.loads(line) for line in sys.stdin if line.strip()]; assert any(t["name"] == "scorecard" for t in lines[0]["result"]); assert "confirm_mutation" in lines[1]["error"]["message"]'
