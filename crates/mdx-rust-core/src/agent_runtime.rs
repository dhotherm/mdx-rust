...
5. Run `mdx-rust --json benchmark --spec .mdx-rust/benchmarks.json` when a benchmark spec exists.
6. Run `mdx-rust --json brief <target>` for a fused agent briefing.
7. Inspect `readiness`, `next_commands`, `security`, contracts, performance, benchmark artifacts, and candidate autonomy decisions.
